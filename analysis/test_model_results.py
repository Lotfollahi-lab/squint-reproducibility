...
def test(config: Dict):
    ...
    results_dir = Path(config['experiment']['wandb_run_dir']) / 'results' / Path(config['model']['model_ckpt_fname']).stem
    ...
    # Get test results
    results = trainer.test(model=model, datamodule=datamodule_batch, verbose=True)

    # Convert to DataFrame and save to the configured results_dir
    df = pd.DataFrame(results)
    df.to_csv(results_dir / "test_metrics.csv", index=False)

    print(f"Test metrics saved to {results_dir}/test_metrics.csv")
