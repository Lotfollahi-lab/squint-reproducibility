...
def _embed_with_uce(
    ...
    # Sanitize before write: pandas may hand us ArrowStringArray columns
    # (default for newer pandas + pyarrow), which the H5AD writer can't
    # serialize. Same shared helper used at the `predicted_adata.h5ad`
    # site.
...
def main() -> None:
    ...
    # 1. Load + concat + UCE embedding extraction (one-time shared
    #    setup). Deterministic inference, so we share the latent
    ...
    print(f"shared setup (load + UCE extraction): {shared_setup_seconds:.1f}s")
    ...
    for s_idx, seed in enumerate(seeds):
        ...
        print("=" * 78)
        # TIMED block: Leiden binary search on the shared UCE latent.
