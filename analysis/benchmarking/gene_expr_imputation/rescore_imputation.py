...
def main(argv=None) -> int:
    ...
    if args.adata is not None:
        ...
        adata_path = args.run_dir / "predicted_adata.h5ad"
        out_dir = args.out_dir or args.run_dir
    else:
        raise SystemExit("Pass --run-dir or --adata.")
    if not adata_path.is_file():
        raise SystemExit(f"adata not found: {adata_path}")

    print(f"Loading {adata_path}")
    adata = ad.read_h5ad(adata_path)
    if "data_split" not in adata.obs.columns:
        raise SystemExit(
            "obs['data_split'] missing — this adata wasn't holdout-split, so the "
            "test-split metrics can't be recomputed.")
    if "X_hat" not in adata.layers:
        raise SystemExit("layers['X_hat'] missing — nothing to score against X.")
